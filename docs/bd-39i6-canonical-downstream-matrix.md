...
| packaged-local-app-path | signed packaged app payload checks and packaged live smoke | `scripts`, `dist/Recordit.app` | `scripts/verify_recordit_release_context.sh`; `scripts/gate_packaged_live_smoke.sh` | `packaged-app` | `retained-rich` | `high-bounded` | `partial` | deterministic fake capture and non-UI launch path | no automated retained drag-install to first launch to first live stop proof on installed DMG artifact | `bd-3co8`, `bd-13tm`, `bd-11vg` |
...
| dmg-install-open | mount DMG, inspect layout, copy/install, open app, retain diagnostics | `scripts`, `README.md` | `scripts/create_recordit_dmg.sh`; `README.md` | `manual-user-journey` | `docs-only` | `low` | `partial` | DMG build exists, but no retained automated verifier | install-surface proof remains open | `bd-3co8`, `bd-13pv`, `bd-11vg` |
