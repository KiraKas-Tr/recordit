...
- this audit captures the historical compatibility boundary as of 2026-03-02.
- `docs/adr-005-recordit-default-entrypoint.md` now supersedes `docs/adr-004-packaged-entrypoint.md` for user-facing default-entrypoint policy.
- packaged `SequoiaTranscribe.app` references below are compatibility/fallback lanes until cutover beads complete.
...
| Packaged run wrappers | `Makefile`: `run-transcribe-app`, `run-transcribe-live-stream-app`, `run-transcribe-preflight-app`, `run-transcribe-model-doctor-app` | signed `SequoiaTranscribe.app` (`transcribe-live`) | `legacy-stable` | keep unchanged | Historical compatibility lane; superseded for user-facing defaults by `ADR-005`. |
...
2. `scripts/gate_packaged_live_smoke.sh` resolves compatibility runtime path expectations through `scripts/resolve_sequoiatranscribe_compat.sh` (default path remains `dist/SequoiaTranscribe.app/Contents/MacOS/SequoiaTranscribe`).
