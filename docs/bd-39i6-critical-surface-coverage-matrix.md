...
Critical-path policy / exception register: `docs/bd-2mbp-no-mock-critical-path-policy.md` and `docs/bd-2mbp-critical-path-exception-register.csv`
...
| Session list / history / search | `app/Services/real_filesystem_session_integration_smoke.swift`; `app/ViewModels/session_list_smoke.swift`; `app/Services/session_search_index_smoke.swift`; `app/Services/jsonl_tailer_smoke.swift`; `app/Services/jsonl_event_surface_smoke.swift` | smoke + integration | `mock` + `fixture` + `temp-filesystem` | search indexing, event mapping, list filtering, real-filesystem detail resolution, and empty/error states | strongest service-level lane now uses real temp session roots, but navigation and some list-level assertions still depend on mock/synthetic seams | no packaged-app lane proves search/history behavior against real session corpus generated by product flows | `bd-10ou`, `bd-11vg`, `bd-2mbp` |
| Export / artifact actions | `app/Services/real_filesystem_session_integration_smoke.swift`; `app/Exports/export_smoke.swift`; `app/Exports/session_export_view_model_smoke.swift`; `app/UIAutomation/ui_automation_smoke.swift` | smoke + integration | `mock` + `fixture` + `temp-filesystem` | export naming, path restrictions, diagnostics redaction, managed-storage policy, and view-model action wiring | strongest service-level lane now exports from real temp session artifacts, but UI-layer export flows still depend on fixture and simulated-service seams | no app-level or packaged lane validating export from real completed sessions in user-facing context | `bd-10ou`, `bd-11vg`, `bd-2mbp` |
